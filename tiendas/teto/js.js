...
function Habilitado(){
  ...
  get(child(dbref, "Usuarios/teto")).then((snapshot) => {
...
function addVista(){
  ...
  get(child(dbref, "Usuarios/teto")).then((snapshot) => {
    ...
    update(ref(realdb, "Usuarios/teto" ),
    {
      zVistas: actVista
...
document.addEventListener('keydown', (event) => {
 if (event.ctrlKey) {
     if (event.keyCode == 65 || event.keyCode == 97) {
         window.location = "epanel-teto.html";
...
function GetAllCategory() {
   ...
   get(child(dbref, "Usuarios/teto/zCategorias")).then((snapshot) => {
       snapshot.forEach(cat => {
...
function GetAllBrand() {
   ...
   get(child(dbref, "Usuarios/teto/zMarcas")).then((snapshot) => {
       snapshot.forEach(cat => {
...
function GetAllColors() {
   const dbref = ref(realdb);
   get(child(dbref, "Usuarios/teto/zColores/Header")).then((snapshot) => {
       snapshot.forEach(cat => {
   ...
   get(child(dbref, "Usuarios/teto/zColores/ShopList")).then((snapshot) => {
       snapshot.forEach(cat => {
   ...
   get(child(dbref, "Usuarios/teto/zColores/Producto")).then((snapshot) => {
       snapshot.forEach(cat => {
   ...
   get(child(dbref, "Usuarios/teto/zColores/Footer")).then((snapshot) => {
       snapshot.forEach(cat => {
...
function GetAllBanners() {
   const dbref = ref(realdb);
   get(child(dbref, "Usuarios/teto/zBannersHeaderLinks")).then((snapshot) => {
       snapshot.forEach(cat => {
...
function GetFont(){
 ...
 get(child(dbref, "Usuarios/teto/zFuente")).then((snapshot) => {
       snapshot.forEach(cat => {
...
if (typeof document.getElementById('btnImgPrincipal') === "object" && document.getElementById('btnImgPrincipal') !== null) {
  ...
  if (typeof document.getElementById('btnImgSecundarias') === "object" && document.getElementById('btnImgSecundarias') !== null) {
      ...
      function GetAllProductsEpanel() {
        const dbref = ref(realdb, "Usuarios/teto/Productos");
      ...
      if (typeof document.getElementById('searchInput') === "object" && document.getElementById('searchInput') !== null) {
        ...
        get(child(dbref, "Usuarios/teto/Productos")).then((snapshot) => {
              snapshot.forEach(cat => {
      ...
      if (typeof document.getElementById('typePago') === "object" && document.getElementById('typePago') !== null) {
        ...
        enviarAWhats.addEventListener('click', function(){
          ...
          update(ref(realdb, "Usuarios/teto/zPedidos/" + totalNroPedidos ),
          {
      ...
      enviarPedido.addEventListener('click',  async function(){
            ...
            canvas.toBlob(async (blob) => {
              ...
              const ImageAdress = "TheImages/teto/Pedidos/" + totalNroPedidos;
