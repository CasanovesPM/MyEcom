...
function addVista(){
  ...
  let ArrayOfVistas = [];
  get(child(dbref, "Usuarios/Maraka")).then((snapshot) => {
    ...
    update(ref(realdb, "Usuarios/Maraka" ),
    {
      zVistas: actVista
...
document.addEventListener('keydown', (event) => {
 if (event.ctrlKey) {
     if (event.keyCode == 65 || event.keyCode == 97) {
         window.location = "epanel-Maraka.html";
...
function GetAllCategory() {
   ...
   get(child(dbref, "Usuarios/Maraka/zCategorias")).then((snapshot) => {
       snapshot.forEach(cat => {
...
function GetAllBrand() {
   ...
   get(child(dbref, "Usuarios/Maraka/zMarcas")).then((snapshot) => {
       snapshot.forEach(cat => {
...
function GetAllColors() {
   const dbref = ref(realdb);
   get(child(dbref, "Usuarios/Maraka/zColores/Header")).then((snapshot) => {
       snapshot.forEach(cat => {
   ...
   get(child(dbref, "Usuarios/Maraka/zColores/ShopList")).then((snapshot) => {
       snapshot.forEach(cat => {
   ...
   get(child(dbref, "Usuarios/Maraka/zColores/Producto")).then((snapshot) => {
       snapshot.forEach(cat => {
   ...
   get(child(dbref, "Usuarios/Maraka/zColores/Footer")).then((snapshot) => {
       snapshot.forEach(cat => {
...
function GetAllBanners() {
   const dbref = ref(realdb);
   get(child(dbref, "Usuarios/Maraka/zBannersHeaderLinks")).then((snapshot) => {
       snapshot.forEach(cat => {
...
function GetFont(){
 ...
 get(child(dbref, "Usuarios/Maraka/zFuente")).then((snapshot) => {
       snapshot.forEach(cat => {
...
if (typeof document.getElementById('btnImgPrincipal') === "object" && document.getElementById('btnImgPrincipal') !== null) {
  ...
  if (typeof document.getElementById('btnImgSecundarias') === "object" && document.getElementById('btnImgSecundarias') !== null) {
      ...
      function GetAllProductsEpanel() {
        const dbref = ref(realdb, "Usuarios/Maraka/Productos");
      ...
      if (typeof document.getElementById('searchInput') === "object" && document.getElementById('searchInput') !== null) {
        ...
        get(child(dbref, "Usuarios/Maraka/Productos")).then((snapshot) => {
              snapshot.forEach(cat => {
      ...
      if (typeof document.getElementById('typePago') === "object" && document.getElementById('typePago') !== null) {
        ...
        enviarAWhats.addEventListener('click', function(){
          ...
          update(ref(realdb, "Usuarios/Maraka/zPedidos/" + totalNroPedidos ),
          {
      ...
      enviarPedido.addEventListener('click',  async function(){
            ...
            canvas.toBlob(async (blob) => {
              ...
              const ImageAdress = "TheImages/Maraka/Pedidos/" + totalNroPedidos;
