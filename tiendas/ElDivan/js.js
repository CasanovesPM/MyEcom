...
function addVista(){
  ...
  let ArrayOfVistas = [];
  get(child(dbref, "Usuarios/ElDivan")).then((snapshot) => {
    ...
    update(ref(realdb, "Usuarios/ElDivan" ),
    {
      zVistas: actVista
...
document.addEventListener('keydown', (event) => {
 if (event.ctrlKey) {
     if (event.keyCode == 65 || event.keyCode == 97) {
         window.location = "epanel-ElDivan.html";
...
function GetAllCategory() {
   ...
   get(child(dbref, "Usuarios/ElDivan/zCategorias")).then((snapshot) => {
       snapshot.forEach(cat => {
...
function GetAllBrand() {
   ...
   get(child(dbref, "Usuarios/ElDivan/zMarcas")).then((snapshot) => {
       snapshot.forEach(cat => {
...
function GetAllColors() {
   const dbref = ref(realdb);
   get(child(dbref, "Usuarios/ElDivan/zColores/Header")).then((snapshot) => {
       snapshot.forEach(cat => {
   ...
   get(child(dbref, "Usuarios/ElDivan/zColores/ShopList")).then((snapshot) => {
       snapshot.forEach(cat => {
   ...
   get(child(dbref, "Usuarios/ElDivan/zColores/Producto")).then((snapshot) => {
       snapshot.forEach(cat => {
   ...
   get(child(dbref, "Usuarios/ElDivan/zColores/Footer")).then((snapshot) => {
       snapshot.forEach(cat => {
...
function GetAllBanners() {
   const dbref = ref(realdb);
   get(child(dbref, "Usuarios/ElDivan/zBannersHeaderLinks")).then((snapshot) => {
       snapshot.forEach(cat => {
...
function GetFont(){
 ...
 get(child(dbref, "Usuarios/ElDivan/zFuente")).then((snapshot) => {
       snapshot.forEach(cat => {
...
if (typeof document.getElementById('btnImgPrincipal') === "object" && document.getElementById('btnImgPrincipal') !== null) {
  ...
  if (typeof document.getElementById('btnImgSecundarias') === "object" && document.getElementById('btnImgSecundarias') !== null) {
      ...
      function GetAllProductsEpanel() {
        const dbref = ref(realdb, "Usuarios/ElDivan/Productos");
      ...
      if (typeof document.getElementById('searchInput') === "object" && document.getElementById('searchInput') !== null) {
        ...
        get(child(dbref, "Usuarios/ElDivan/Productos")).then((snapshot) => {
              snapshot.forEach(cat => {
      ...
      if (typeof document.getElementById('typePago') === "object" && document.getElementById('typePago') !== null) {
        ...
        enviarAWhats.addEventListener('click', function(){
          ...
          update(ref(realdb, "Usuarios/ElDivan/zPedidos/" + totalNroPedidos ),
          {
      ...
      enviarPedido.addEventListener('click',  async function(){
            ...
            canvas.toBlob(async (blob) => {
              ...
              const ImageAdress = "TheImages/ElDivan/Pedidos/" + totalNroPedidos;
